...
Before running Panorama-seq, convert the ground-truth spot coordinates to the barcode CSV format expected by the pipeline (one row per unique spot, columns `barcode,x,y`):

```bash
python3 scripts/read_to_spot_csv.py \
    --read_to_spot  results/<run>/read_to_spot.tsv \
    --spot_coords   results/<run>/spot_coordinates.tsv \
    --out           results/<run>/barcodes_coords.csv
```
